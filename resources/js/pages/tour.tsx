    ...
    return (
        <Main>
            ...
            <section className="page-header">
                ...
                <p className="page-subtitle">Experience Lunar Blood live - dark atmospheres and crushing riffs in intimate venues</p>
            </section>

            <section className="mb-12">
                <div className="space-y-4">
                    {tourDates.map((show) => (
                        <div key={show.id} className="card transition-shadow hover:shadow-xl">
                            {/* Mobile Layout */}
                            <div className="block space-y-3 md:hidden">
                                <div className="flex items-start justify-between">
                                    ...
                                    <span className={`text-sm font-medium ${getStatusColor(show.status)}`}>{getStatusText(show.status)}</span>
                                </div>
                                <div>
                                    ...
                                    <div className="ml-6 text-[var(--muted-foreground)]">{show.venue}</div>
                                </div>
                                <div>
                                    {show.status === 'on-sale' && show.ticketUrl ? (
                                        <a href={show.ticketUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary w-full">
                                            Buy Tickets
                                        </a>
                            ...
                            <div className="hidden grid-cols-12 items-center gap-4 md:grid">
                                ...
                                <div className="col-span-2 text-right">
                                    {show.status === 'on-sale' && show.ticketUrl ? (
                                        <a href={show.ticketUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary">
                                            Buy Tickets
                                        </a>
                                    ) : show.status === 'sold-out' ? (
            ...
            <section className="grid gap-8 md:grid-cols-2">
                <div className="card">
                    ...
                    <p className="mb-4 text-[var(--muted-foreground)]">
                        Get exclusive access with our VIP experience packages including meet & greet, signed merchandise, and early venue entry.
                    </p>
